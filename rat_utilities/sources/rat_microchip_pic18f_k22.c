...
// ----------------------------------------------------------------------------------------------------
// Microchip PIC18F K22 Source File
// ----------------------------------------------------------------------------------------------------
...
// Debug mode
//
// The default ports of the software UART are RA2 (TX) and RA3 (RX) and
// the default baud rate is 9,600 bps.
//
// Note that the RX port is not currently used.
//
// The debug mode must NOT have any affect on the production releases or
...
static void rat_init_debugging (void)
{
  ...
  // --------------------------------------------------------------------------------------------------
  #ifdef TEST_MODE
    software_uart_code = Soft_UART_Init(DEBUG_PORT,
                                        DEBUG_RX,
                                        DEBUG_TX,
                                        DEBUG_BAUD_RATE,
                                        0);
